...
export class View extends mix(Dispatch).with(Events) {
  constructor({type = null, selector = null, dimensions = {}, spec = null, renderer = null, build = () => {}} = {}) {
    super()
    this.svg, this.canvas, this.width, this.height, this.margin

    this.name = this._generateName(type)
  ...
  _generateName(type) {
    ...
    return _.uniqueId(type ? name_prefix + `${type}-` : name_prefix)
  }
